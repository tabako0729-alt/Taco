...
const BUILD_TAG = '2026-07-11-v5-adname-fix'; // 全新唯一 tag，专治版本探针假阳性（旧 commit 也写过 v4-size）
...
async function tacoTalk(text) {
  const t = (text || '').trim();
  // ① 廉价意图（不打飞书，零延迟）：预订 / 转人工 / 问候 —— 必须优先于数据意图
  if (/(预订|下单|购买|锁|留)/.test(t)) return '预订请走官网完成乐观下单；或告诉我具体广告位名称，我帮你确认库存。';
  if (/(人工|客服|转|真人|顾问)/.test(t)) return '__TRANSFER__';
  if (/(你好|hi|hello|在吗)/i.test(t)) return '你好，我是 Taco 智能体 🌮 可查广告位库存、报价与尺寸。试试：「有哪些广告位？」「1/3版平面广告 尺寸」';
  // ② 数据意图才打飞书（库存/报价/尺寸/具体广告位名）
  const needData = /(库存|有什么|报价|价格|多少钱|广告位|刊例|还有吗|有啥|可买|在售|列表|尺寸|规格|多大|格式|dpi|分辨率|素材|提交|1\/3|封底|通栏|整版|半版|专访)/i.test(t);
  if (!needData) return '我是 Taco 智能体，可查广告位库存、报价与尺寸规格。试试：「有哪些广告位？」「1/3版平面广告 尺寸」。';
  try {
    const recs = await feishuList(TBL_INVENTORY);
    // ③ 具体广告位名匹配（最高优先级）：查询包含某广告位全名 → 返回该位详情
    const hit = recs
      .filter(r => { const n = r.fields['资源名称'] || ''; return n && t.includes(n); })
      .sort((a, b) => (a.fields['资源名称'] || '').length - (b.fields['资源名称'] || '').length)[0];
    if (hit) {
      const f = hit.fields;
      return `📐 ${f['资源名称']}\n类型：${f['媒体类型'] || ''}｜位置：${f['版面/位置'] || ''}\n状态：${f['当前状态']}\n刊例价：${f['刊例价格'] || ''}元\n规格：${f['备注规格'] || '暂无规格说明'}`;
    }
    // ④ 尺寸/规格通用意图：没匹配到具体广告位名 → 列出常见尺寸
    if (/(尺寸|规格|多大|格式|dpi|分辨率|mm|cm|像素|长边|素材|提交)/i.test(t)) {
      const sizes = recs
        .filter(r => /尺寸/.test(r.fields['备注规格'] || ''))
        .map(r => `· ${r.fields['资源名称']}：${(r.fields['备注规格'] || '').match(/尺寸：[^；]*/)?.[0] || ''}`)
        .join('\n');
      return `请问你想查哪个广告位的尺寸？报纸广告位常见尺寸：\n${sizes}\n\n直接说「广告位名+尺寸」即可，如「1/3版平面广告 尺寸」。`;
    }
    // ⑤ 库存/报价/列表意图
...
async function handleChatApi(request) {
  ...
  return json({ reply, v: BUILD_TAG }, 200); // v 字段用于一验便知线上是否部署了本版
}
