...
const GLM_API_KEY = 'TODO_FILL_YOUR_GLM_KEY'; // 部署前在边缘函数填入智谱 GLM key
...
const TACO_TOOLS = [
  { type:'function', function:{ name:'get_inventory', description:'读取飞书广告位库存（实时）。用于查询/价格/规格/最贵最便宜/含美工过滤/按媒体类型或期数或位置筛选/推荐前取数。', parameters:{ type:'object', properties:{ filter_status:{type:'string',description:'按状态过滤，如「可购买」'}, media_type:{type:'string',description:'媒体类型，如「报纸」「公众号」'}, period:{type:'string',description:'期数，如「第三期」'}, has_art:{type:'string',enum:['含美工','不含美工']}, location:{type:'string',description:'版面位置关键词，如「封底」'} } } } },
  ...
  { type:'function', function:{ name:'write_order', description:'写入一条订单（接单动作）。写单前槽位必须收齐。', parameters:{ type:'object', properties:{ itemId:{type:'string'}, enterprise:{type:'string'}, region:{type:'string'}, amount:{type:'number',description:'成交金额（元，如150000）'}, contact:{type:'string'}, phone:{type:'string'}, period:{type:'string'} }, required:['itemId','enterprise','region','amount','contact','phone','period'] } } },
];
...
async function tacoToolImpl(name, args) {
  ...
  if (name === 'write_order') {
    ...
    if (typeof args.itemId === 'string' && args.itemId.startsWith('rec')) invRecId = args.itemId;
      ...
      if (!invRecs.length) throw new Error('未找到资源ID对应的广告位记录：' + args.itemId);
      invRecId = invRecs[0].record_id;
    }
    const fields = {
      '关联广告位': [invRecId],
...
async function tacoBrain(text) {
  ...
  for (let step = 0; step < 5; step++) {
    const r = await fetch(GLM_ENDPOINT, { method:'POST', headers:{'Content-Type':'application/json', Authorization:'Bearer '+GLM_API_KEY}, body: JSON.stringify({ model:GLM_MODEL, messages, tools:TACO_TOOLS, tool_choice:'auto', temperature:0.3 }) });
